...
# Fully Connected Layers without station information

data_path = "/Users/amritnijjar/Desktop/mta_data_project_2/data/"

X_train = np.load(data_path + "X_train.npy")[:, 2:]
y_train = np.load(data_path + "y_train.npy")

X_val = np.load(data_path + "X_val.npy")[:, 2:]
...
for depth in [1, 2, 3]:
    for width in [24, 48, 96]:
        ...
        model_ = construct_model(in_size = 24, out_size = 6, hidden_depth = depth, hidden_width = width)
        optimizer_ = optim.Adam(model_.parameters(), lr = 0.001)
        loss_fn_ = nn.L1Loss()
        n_epochs_ = 50

        trainer = ModelTrainer(model = model_, n_epochs = n_epochs_, optimizer = optimizer_, loss_fn = loss_fn_, save_loc = "/Users/amritnijjar/Desktop/mta_data_project_2/models/FF_no_station/")

        trainer.train(train_loader, val_loader)

        trainer.save(f"FF_no_station_depth{depth}_width{width}_batch{batch_size_}")
